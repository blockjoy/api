...
async fn node_can_create_secrets() {
    ...
    let req = api::CryptServicePutSecretRequest {
        resource: common::Resource::Host as i32,
        resource_id: test.seed().host.id.to_string(),
        name: test_name!().into(),
    ...
    let req = api::CryptServicePutSecretRequest {
        resource: common::Resource::Node as i32,
    ...
    let req = api::CryptServicePutSecretRequest {
        resource: common::Resource::Node as i32,
        resource_id: test.seed().node.id.to_string(),
        name: test_name!().into(),
        value: TEST_SECRET.to_vec(),
    };
...
async fn delete_node_deletes_secrets() {
    ...
    let node_id: NodeId = node.id.parse().unwrap();
...
async fn new_node_with_old_id_copies_secrets() {
    ...
    let new_node_id: NodeId = new_node.id.parse().unwrap();
